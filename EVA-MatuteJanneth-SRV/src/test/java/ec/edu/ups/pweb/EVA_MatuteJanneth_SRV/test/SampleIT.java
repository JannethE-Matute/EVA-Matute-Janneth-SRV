package ec.edu.ups.pweb.EVA_MatuteJanneth_SRV.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/EVA-MatuteJanneth-SRV.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "EVA-MatuteJanneth-SRV.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.pweb.EVA_MatuteJanneth_SRV.test");
